...
import transforms
import utils
from PolyRelu import PolyRelu, get_penalty, get_metrics, generate_coeffs, real_relu, PolynomialEvaluator
...
l_infinities = {0: deque(maxlen=maxlen),
                1: deque(maxlen=maxlen),
                2: deque(maxlen=maxlen)}
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None):
    ...
    for i, (image, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            output = model(image)
            if args.use_poly:
                # we get the l_infinities at each layer and add them to the queue buffer
                for j in PolyRelu.ids:
                    buffer = PolyRelu.ordered_buffer[j]
                    np_buffer = torch.cat(buffer).cpu().detach().flatten().numpy()
                    l_inf = np.linalg.norm(np_buffer, ord=np.inf)
                    l_infinities[j].append(l_inf)
                PolyRelu.reset()
    ...
    if np.isnan(metric_logger.meters["loss"].global_avg):
        print("Model died duriag training due to a NaN loss")
        return False
    else:
        return True
...
def main(args):
    ...
    if args.use_poly:
        ...
        module = PolyRelu  # a polynomial relu that saves the inputs it gets fed
        ...
        utils.special_strip(model, "relu", module, torch.nn.ReLU, **kwargs)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not has_not_died:
            break
        lr_scheduler.step()
        acc = evaluate(model, criterion, data_loader_test, args=args, device=device)
        if acc > best_acc:
            best_acc = acc
            if args.output_dir:
                checkpoint = {
        ...
        if model_ema:
            ema_acc = evaluate(model_ema, criterion, data_loader_test, args=args, device=device, log_suffix="EMA")
    ...
    if args.use_poly:
        # we save the last buffers to a pickle so we can load them and plot them
        import pickle
        utils.mkdir("./experiments/figure_2/")
        with open('./experiments/figure_2/l_infinities.pkl', 'wb') as f:
            pickle.dump(l_infinities, f)
        with open('./experiments/figure_2/maxlen.pkl', 'wb') as f:
            pickle.dump(maxlen, f)

    if args.use_poly and has_not_died:
        if args.dataset == "imagenet":
            device = torch.device("cpu")
        print("Last model metrics")
        get_metrics(dataloader=data_loader_test, model=model, device=device, input_range=(-args.range, args.range),
                    metrics=["oor_percentage", "num_oor_total", "unique_image_with_oor_total", 'oor_max', 'oor_values'])
        if os.path.exists(os.path.join(args.output_dir, "best_ema_model.pth")):
            ...
            get_metrics(dataloader=data_loader_test, model=model_ema, device=device, input_range=(-args.range, args.range),
                        metrics=["oor_percentage", "num_oor_total", "unique_image_with_oor_total", 'oor_max', 'oor_values'])
        if os.path.exists(os.path.join(args.output_dir, f"best_model.pth")):
            print("Best model metrics")
            checkpoint = torch.load(os.path.join(args.output_dir, f"best_model.pth"), map_location="cpu")
            model_without_ddp.load_state_dict(checkpoint["model"])
            get_metrics(dataloader=data_loader_test, model=model, device=device, input_range=(-args.range, args.range),
                        metrics=["oor_percentage", "num_oor_total", "unique_image_with_oor_total", 'oor_max', 'oor_values'])
    else:
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--data-path", default="/scratch/lprfenau/datasets/imagenet/", type=str, help="dataset path")
